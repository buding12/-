...
答: 1、git init初始化git仓库（新项目才有这一步）
    ...
    3、git checkout -b dev (dev 为本地分支名） checkout 切割啊特
    4、git add .将工作区文件存在暂存区
    4、git commit -m  ""从暂存区存到仓储区
    5、git checkout master切到master分支   master 妈死特
    6、git merge dev 合并分支,合并后要将分支删除   merge 吗儿G
...
```js
 ...
 特点：
    1、Promise对象只有三种状态。
        异步操作“未完成”（pending）攀盯
        异步操作“已完成”（resolved 瑞绕， 又称 fulfilled）佛飞哟
        异步操作“失败”（rejected）瑞JK特
        异步操作成功，Promise对象传回一个值，状态变为 resolved。瑞绕
        异步操作失败，Promise对象抛出一个错误，状态变为rejected。瑞杰克特
...
3、Promise.prototype.finally()
	1）烦得雷 finally方法用于指定不管 Promise 对象最后状态如何，都会执行的回调函数。
...
```js
    ...
    2、同步和异步任务分别进入不同的执行环境，同步的进入主线程，即主执行栈，异步的进入    E瘟特 Event 秋 Queue（事件队列） 。主线程内的任务执行完毕为空，会去 Event Queue 读取对应的任务，推入主线程执行。 上述过程的不断重复就是我们说的 Event 漏扑 Loop (事件循环)。
```
